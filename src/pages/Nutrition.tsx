    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-6xl mx-auto pb-10">
      <Tabs defaultValue="dashboard" className="w-full">
        ...
        <TabsContent value="dashboard" className="space-y-6">
          <div className="grid xl:grid-cols-3 lg:grid-cols-3 md:grid-cols-2 grid-cols-1 gap-6">
            ...
            <div className="space-y-6 flex flex-col">
              <div className="bg-[#111111]/85 backdrop-blur-md rounded-3xl p-6 border border-white/5 relative overflow-hidden shadow-xl">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <div>
                      <h2 className="text-xs font-black text-white uppercase tracking-widest">{t('smart_nutrition')}</h2>
                      <p className="text-[9px] text-slate-500 font-bold uppercase tracking-widest mt-0.5">{t('realtime_metabolism')}</p>
                   </div>
                </div>

                <div className="flex justify-between items-end mb-2">
                   <div className="flex items-end gap-2">
                      <span className="text-5xl font-black text-white leading-none tracking-tight">{consumedKcal}</span>
                      <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('kcal_consumed')}</span>
                   </div>
                   <div className="text-right">
                      <span className="text-xl font-black text-slate-300">/ {targetKcal}</span>
                      <p className="text-[9px] uppercase font-bold tracking-widest text-slate-500 mt-0.5">{t('daily_limit')}</p>
                   </div>
                </div>
                <Progress value={(consumedKcal / targetKcal) * 100 || 0} className="h-2 bg-white/5 mb-6 overflow-hidden [&>div]:bg-gradient-to-r [&>div]:from-cyan-400 [&>div]:to-indigo-500 shadow-[0_0_10px_rgba(34,211,238,0.25)] rounded-full" />

                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-gradient-to-br from-[#1a1a2e]/40 to-[#0e0e1a]/60 rounded-2xl p-4 border border-white/5 flex flex-col justify-between">
                    <div>
                      <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-1">{t('remaining')}</p>
                      <p className={`text-xl font-black mb-3 ${remainingKcal < 0 ? 'text-red-400' : 'text-cyan-400'}`}>{remainingKcal} KCAL</p>
                    </div>
                    <Button 
                      onClick={() => setShowRecipeGen(true)}
                      className="w-full h-9 bg-purple-500/10 text-purple-300 hover:bg-purple-500/25 text-[9px] font-black uppercase tracking-widest rounded-xl border border-purple-500/20 shadow-sm transition-all"
                    >
                      <Sparkles className="w-3.5 h-3.5 mr-2" /> Find Recipe
                    </Button>
                  </div>
                  
                  <div className="bg-gradient-to-br from-[#1a1a2e]/40 to-[#0e0e1a]/60 rounded-2xl p-4 border border-white/5 flex flex-col justify-center space-y-3">
                    {[
                      ...
                      <div key={m.n}>
                        <div className="flex justify-between text-[9px] font-black uppercase tracking-widest mb-1.5">
                          ...
                          <span className="text-white">{m.v}g <span className="text-slate-600">/ {m.t}g</span></span>
                        </div>
                        <Progress value={(m.v / m.t) * 100 || 0} className={`h-1.5 bg-white/5 rounded-full overflow-hidden [&>div]:${m.color}`} />
                      </div>
                    ))}
                  </div>
                </div>
              ...
              <div className="bg-[#111111]/80 backdrop-blur-md rounded-3xl p-6 border border-white/5 flex-1 flex flex-col shadow-xl">
                <div className="flex items-center gap-3 mb-6">
                   <div className="w-10 h-10 rounded-2xl bg-orange-500/10 border border-orange-500/25 flex items-center justify-center text-orange-400">
                      <Utensils className="w-5 h-5" />
                   </div>
                   <h2 className="text-xs font-black text-white uppercase tracking-widest">{t('nutrition_db_search')}</h2>
                </div>
                ...
                <div className="flex gap-2.5 mb-4">
                  <div className="relative flex-1">
                    <Input 
                      ...
                      className="bg-black/50 border-white/10 h-11 rounded-xl text-xs pl-10 focus-visible:ring-cyan-500/50 focus-visible:border-cyan-500/30 text-white placeholder:text-slate-500" 
                      value={searchQuery}
                      onChange={e => setSearchQuery(e.target.value)}
                      onKeyDown={e => e.key === 'Enter' && handleSearch()}
                    />
                    <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="absolute left-3.5 top-3.5 text-slate-500"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                  </div>
                  <Button 
                    onClick={() => setShowScanner(true)}
                    className="bg-white/5 text-white hover:bg-white/10 hover:border-white/20 h-11 rounded-xl px-3.5 border border-white/10 transition-all shrink-0"
                  >
                  ...
                  <Button 
                    ...
                    className="bg-cyan-950/40 text-cyan-400 hover:bg-cyan-900/40 h-11 rounded-xl px-6 text-xs font-black uppercase tracking-widest border border-cyan-500/20 transition-all shrink-0">
                    {isSearching ? <Loader2 className="w-4 h-4 animate-spin" /> : t('search_btn')}
                  </Button>
                </div>
                <p className="text-[8px] text-cyan-400/60 font-black uppercase tracking-widest text-center mb-6">{t('ai_nutrition_note')}</p>
                ...
                <div className="flex-1 border border-white/5 bg-black/20 rounded-2xl flex flex-col items-center justify-center gap-4 text-slate-500 p-8 min-h-[150px]">
                  {isSearching ? (
                    <div className="flex flex-col items-center gap-4">
                      ...
                      <p className="text-xs font-black uppercase tracking-wider text-center text-cyan-400/80">{t('analyzing_food')}</p>
                    </div>
                  ) : searchResult ? (
                    <div className="w-full h-full flex flex-col animate-in fade-in duration-300">
                       <h3 className="text-sm font-black text-white capitalize text-center mb-4">{searchResult.name}</h3>
                       
                       <div className="flex items-center gap-3 mb-4 bg-black/30 p-2 rounded-xl border border-white/5">
                         <Label className="text-[9px] font-black text-slate-500 uppercase tracking-widest whitespace-nowrap pl-2">{t('amount_g')}</Label>
                         <Input 
                           type="number" 
                           value={consumedGram} 
                           onChange={e => setConsumedGram(e.target.value === "" ? "" : Number(e.target.value))}
                           className="bg-black/50 border-white/10 h-9 rounded-lg text-center focus-visible:ring-cyan-500/50 text-white font-black text-xs"
                         />
                       </div>

                       <div className="grid grid-cols-2 gap-3 flex-1">
                          <div className="bg-gradient-to-br from-[#1a1a2e]/40 to-[#0e0e1a]/60 rounded-xl border border-white/5 p-3 text-center flex flex-col justify-center">
                            <span className="text-xl font-black text-cyan-400">{Math.round(searchResult.kcal * (Number(consumedGram) || 0) / 100)}</span>
                            <span className="text-[8px] font-bold text-slate-500 uppercase tracking-widest mt-1">Kcal</span>
                          </div>
                          <div className="bg-gradient-to-br from-[#1a1a2e]/40 to-[#0e0e1a]/60 rounded-xl border border-white/5 p-3 text-center flex flex-col justify-center">
                            <span className="text-lg font-black text-white">{Math.round(searchResult.protein * (Number(consumedGram) || 0) / 100)}g</span>
                            <span className="text-[8px] font-bold text-cyan-500/70 uppercase tracking-widest mt-1">{t('protein')}</span>
                          </div>
                          <div className="bg-gradient-to-br from-[#1a1a2e]/40 to-[#0e0e1a]/60 rounded-xl border border-white/5 p-3 text-center flex flex-col justify-center">
                            <span className="text-lg font-black text-white">{Math.round(searchResult.carbs * (Number(consumedGram) || 0) / 100)}g</span>
                            <span className="text-[8px] font-bold text-yellow-500/70 uppercase tracking-widest mt-1">{t('carbs')}</span>
                          </div>
                          <div className="bg-gradient-to-br from-[#1a1a2e]/40 to-[#0e0e1a]/60 rounded-xl border border-white/5 p-3 text-center flex flex-col justify-center">
                            <span className="text-lg font-black text-white">{Math.round(searchResult.fat * (Number(consumedGram) || 0) / 100)}g</span>
                            <span className="text-[8px] font-bold text-pink-500/70 uppercase tracking-widest mt-1">{t('fats')}</span>
                          </div>
                       </div>
                       <Button onClick={() => { 
                           ...
                           setSearchResult(null); 
                           setSearchQuery(""); 
                         }} className="mt-4 w-full h-10 bg-cyan-500/20 text-cyan-400 hover:bg-cyan-500/30 border border-cyan-500/20 font-black uppercase tracking-widest text-xs rounded-xl shadow-md transition-all">
                         {t('log_db_item')}
                       </Button>
                    </div>
                  ) : (
                    <>
                       <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" className="text-slate-600"><circle cx="11" cy="11" r="8"></circle><line x1="21" y1="21" x2="16.65" y2="16.65"></line></svg>
                       <p className="text-[10px] font-bold uppercase tracking-widest text-center text-slate-500 leading-relaxed max-w-xs">{t('enter_food_ai')}</p>
                    </>
                  )}
                </div>
              </div>
            ...
            {/* Column 2: Food Diary */}
            <div className="bg-[#111111]/80 backdrop-blur-md rounded-3xl p-6 border border-white/5 flex flex-col max-h-[800px] overflow-hidden shadow-xl">
              <div className="flex flex-col items-center mb-6 pt-2">
                ...
                <div className="flex items-center w-full justify-center relative">
                  <h2 className="text-sm font-black text-white uppercase tracking-widest italic">{t('metabolic_ledger')}</h2>
                  {diary.length > 0 && (
                    <Button variant="ghost" size="sm" onClick={clearNutritionDiary} className="absolute right-0 text-red-400 hover:text-red-300 hover:bg-red-500/10 text-[9px] uppercase font-bold tracking-widest rounded-lg h-8 border border-red-500/5">
                      {t('clear')}
                    </Button>
                  )}
                </div>
              ...
              <div className="flex-1 overflow-y-auto pr-2 space-y-6 mb-4 custom-scrollbar">
                {diary.length === 0 ? (
                  <div className="text-center text-slate-500 text-[10px] font-black uppercase tracking-widest mt-10">{t('no_entries_today')}</div>
                ) : (
                  (['breakfast', 'lunch', 'dinner', 'snack'] as const).map(mType => {
                    ...
                    return (
                      <div key={mType} className="space-y-2.5">
                        <div className="flex justify-between items-center bg-white/[0.02] border border-white/5 rounded-2xl px-4 py-2.5">
                          <span className="text-[10px] font-black text-white uppercase tracking-widest flex items-center gap-2">
                            <span>{icon}</span>
                            <span>{label}</span>
                            <span className="text-[9px] font-bold text-slate-500 bg-white/5 px-2.5 py-0.5 rounded-full">{foods.length} món</span>
                          </span>
                        ...
                        <div className="space-y-2 pl-2">
                          {foods.length === 0 ? (
                            <div 
                              ...
                              className="text-[9px] text-slate-500 font-black uppercase tracking-widest py-2.5 px-4 border border-dashed border-white/5 rounded-xl hover:border-cyan-500/25 hover:bg-cyan-500/5 cursor-pointer transition-all flex items-center justify-between"
                            >
                            ...
                            foods.map((food) => {
                              ...
                              return (
                                <div key={food.id} className="group relative bg-[#1c1c2e]/40 border border-white/5 rounded-2xl p-4 flex justify-between items-center transition-all duration-300 hover:bg-[#1c1c2e]/70 hover:border-purple-500/30">
                                   <div>
                                      <div className="text-xs font-black text-white capitalize">{food.name}</div>
                                      <div className="flex gap-2 mt-1.5">
                                         <span className="text-[8px] font-black text-cyan-400 bg-cyan-500/10 px-1.5 py-0.5 rounded">P: {food.protein}g</span>
                                         <span className="text-[8px] font-black text-yellow-400 bg-yellow-500/10 px-1.5 py-0.5 rounded">C: {food.carbs}g</span>
                                         <span className="text-[8px] font-black text-pink-400 bg-pink-500/10 px-1.5 py-0.5 rounded">F: {food.fat}g</span>
                                      </div>
                                   </div>
                                   <div className="flex items-center gap-3">
                                      ...
                                      <button 
                                        ...
                                        className="opacity-0 group-hover:opacity-100 p-2 rounded-xl text-slate-500 hover:bg-red-500/10 hover:text-red-400 transition-all duration-200"
                                      >
              ...
              <div className="pt-4 border-t border-white/5">
                <div className="relative mb-4 text-center">
                  ...
                  <span className="relative bg-[#111111] px-4 text-[9px] text-slate-500 font-black uppercase tracking-widest">{t('manual_entry')}</span>
                </div>
                ...
                <div className="space-y-3">
                  <Input value={manualName} onChange={e=>setManualName(e.target.value)} placeholder={t('food_item_placeholder')} className="bg-black/30 border-white/5 h-11 rounded-xl text-xs font-semibold text-center placeholder:text-slate-600 focus-visible:ring-purple-500/30" />
                  <div className="grid grid-cols-4 gap-2">
                    <Input value={manualPro} onChange={e=>setManualPro(e.target.value)} placeholder={t('protein') + " (g)"} type="number" className="bg-black/30 border-white/5 h-11 rounded-xl text-[10px] font-semibold text-center placeholder:text-slate-600 px-1 focus-visible:ring-purple-500/30" />
                    <Input value={manualCarb} onChange={e=>setManualCarb(e.target.value)} placeholder={t('carbs') + " (g)"} type="number" className="bg-black/30 border-white/5 h-11 rounded-xl text-[10px] font-semibold text-center placeholder:text-slate-600 px-1 focus-visible:ring-purple-500/30" />
                    <Input value={manualFat} onChange={e=>setManualFat(e.target.value)} placeholder={t('fats') + " (g)"} type="number" className="bg-black/30 border-white/5 h-11 rounded-xl text-[10px] font-semibold text-center placeholder:text-slate-600 px-1 focus-visible:ring-purple-500/30" />
                    <Input value={manualKcal} onChange={e=>setManualKcal(e.target.value)} placeholder="Kcal" type="number" className="bg-black/30 border-white/5 h-11 rounded-xl text-[10px] font-semibold text-center placeholder:text-slate-600 px-1 focus-visible:ring-purple-500/30" />
                  </div>
                </div>

                <Button onClick={handleManualAdd} className="w-full h-11 bg-purple-500/20 hover:bg-purple-500/30 text-purple-400 border border-purple-500/20 font-black text-xs uppercase tracking-widest rounded-xl mt-4 transition-all">
                  {t('log_entry')}
                </Button>
              </div>
            </div>
            ...
            <div className="space-y-6 flex flex-col">
              <div className="bg-[#111111]/80 backdrop-blur-md rounded-3xl p-6 border border-white/5 shadow-xl relative overflow-hidden">
                <div className="flex items-center gap-3 mb-6">
                   <div className="w-8 h-8 rounded-xl bg-orange-500/10 border border-orange-500/20 flex items-center justify-center text-orange-400">
                      <Flame className="w-4 h-4" />
                   </div>
                   <h2 className="text-xs font-black text-white uppercase tracking-widest">{t('todays_motivation')}</h2>
                </div>
              ...
              <div className="bg-[#111111]/80 backdrop-blur-md rounded-3xl p-6 border border-white/5 flex flex-col shadow-xl">
                <div className="flex items-center justify-between w-full relative mb-6">
                   <div className="flex items-center gap-3">
                      ...
                      <h2 className="text-xs font-black text-white uppercase tracking-widest">{t('hydration_tracker')}</h2>
                   </div>
                   {waterLiter > 0 && (
                     <Button variant="ghost" size="sm" onClick={resetWater} className="text-red-400 hover:text-red-300 hover:bg-red-500/10 text-[9px] uppercase font-black tracking-widest rounded-lg h-7 px-2.5 border border-red-500/10">
                       Reset
                     </Button>
                   )}
                </div>
                ...
                <div className="flex items-center justify-center mb-6">
                   <div className="w-32 h-32 rounded-full border-4 border-white/5 flex items-center justify-center relative overflow-hidden bg-[#0d1527]/30 shadow-inner">
                     ...
                     <div className="relative z-10 text-center">
                       <span className="text-3xl font-black text-white leading-none tracking-tight">{waterLiter.toFixed(2)}</span>
                       <p className="text-[9px] font-bold text-blue-400 uppercase tracking-widest mt-1">/ {waterTarget.toFixed(2)}L</p>
                     </div>
                   </div>
                </div>

                <div className="grid grid-cols-3 gap-2.5 mb-4">
                  <Button onClick={() => addWater(0.25)} className="h-14 bg-black/40 text-slate-300 border border-white/5 hover:bg-white/5 hover:border-blue-500/30 rounded-2xl text-[10px] flex flex-col items-center justify-center gap-1 font-black transition-all hover:shadow-[0_0_15px_rgba(59,130,246,0.1)]">
                    <span className="text-sm">🥛</span>
                    <span>250ml</span>
                  </Button>
                  <Button onClick={() => addWater(0.50)} className="h-14 bg-black/40 text-slate-300 border border-white/5 hover:bg-white/5 hover:border-blue-500/30 rounded-2xl text-[10px] flex flex-col items-center justify-center gap-1 font-black transition-all hover:shadow-[0_0_15px_rgba(59,130,246,0.1)]">
                    <span className="text-sm">🧴</span>
                    <span>500ml</span>
                  </Button>
                  <Button onClick={() => addWater(0.75)} className="h-14 bg-blue-500/10 text-blue-400 border border-blue-500/20 hover:bg-blue-500/20 rounded-2xl text-[10px] flex flex-col items-center justify-center gap-1 font-black transition-all hover:shadow-[0_0_15px_rgba(59,130,246,0.15)]">
                    <span className="text-sm">🥤</span>
                    <span>750ml</span>
                  </Button>
                </div>
                ...
                <div className="flex gap-2 pt-3 border-t border-white/5">
                  <Input 
                    ...
                    className="bg-black/40 border-white/10 h-10 rounded-xl text-xs text-center focus-visible:ring-blue-500/50 focus-visible:border-blue-500/30 text-white font-black"
                  />
                  <Button 
                    onClick={() => {
                    ...
                    className="bg-blue-500/20 text-blue-400 hover:bg-blue-500/30 border border-blue-500/20 h-10 rounded-xl px-4 text-xs font-black uppercase tracking-widest whitespace-nowrap"
                  >
                    + Nạp
                  </Button>
                </div>
              ...
              <div className="bg-[#111111]/80 backdrop-blur-md rounded-3xl p-6 border border-white/5 shadow-xl">
                <div className="flex items-center gap-3 mb-6">
                   ...
                   <h2 className="text-xs font-black text-white uppercase tracking-widest">{t('todays_summary')}</h2>
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="bg-black/30 rounded-2xl p-4 border border-white/5 text-center">
                    ...
                    <p className="text-lg font-black text-white">{consumedKcal} <span className="text-xs font-bold text-slate-500">/{targetKcal}</span></p>
                  </div>
                  <div className="bg-black/30 rounded-2xl p-4 border border-white/5 text-center">
                    ...
                    <p className="text-lg font-black text-white">{consumedPro}g <span className="text-xs font-bold text-slate-500">/{targetPro}g</span></p>
                  </div>
                </div>
              </div>
        ...
        <TabsContent value="calculator" className="space-y-6">
          <Card className="bg-[#111111]/80 border-white/5 rounded-[2rem] shadow-xl overflow-hidden xl:grid xl:grid-cols-[1.2fr_1fr] backdrop-blur-md">
             <div className="p-6 lg:p-10 space-y-8 relative">
                ...
                <div className="grid sm:grid-cols-2 gap-6">
                   <div className="space-y-2">
                      ...
                      <div className="grid grid-cols-2 gap-3 h-14">
                        <button onClick={() => setCalcGender('male')} className={`rounded-2xl text-xs font-black uppercase tracking-wider border transition-all duration-300 ${calcGender === 'male' ? 'bg-cyan-500/20 text-cyan-400 border-cyan-500/50 shadow-[0_0_15px_rgba(34,211,238,0.2)]' : 'bg-black/40 text-slate-500 border-white/5 hover:border-white/10'}`}>{t('male')}</button>
                        <button onClick={() => setCalcGender('female')} className={`rounded-2xl text-xs font-black uppercase tracking-wider border transition-all duration-300 ${calcGender === 'female' ? 'bg-pink-500/20 text-pink-400 border-pink-500/50 shadow-[0_0_15px_rgba(244,63,94,0.2)]' : 'bg-black/40 text-slate-500 border-white/5 hover:border-white/10'}`}>{t('female')}</button>
                      </div>
                   </div>
                   <div className="space-y-2">
                ...
                <div className="space-y-3 pt-4 border-t border-white/5">
                   ...
                   <div className="grid grid-cols-3 gap-3">
                      <button onClick={() => setCalcGoal("lose")} className={`h-14 rounded-[1.25rem] text-xs font-black uppercase tracking-wider border transition-all duration-300 ${calcGoal === "lose" ? 'bg-orange-500/20 text-orange-400 border-orange-500/50 shadow-[0_0_15px_rgba(249,115,22,0.15)]' : 'bg-black/40 text-slate-400 border-white/5 hover:border-white/20'}`}>{t('lose_fat')}</button>
                      <button onClick={() => setCalcGoal("maintain")} className={`h-14 rounded-[1.25rem] text-xs font-black uppercase tracking-wider border transition-all duration-300 ${calcGoal === "maintain" ? 'bg-slate-500/20 text-slate-300 border-slate-500/50 shadow-[0_0_15px_rgba(100,116,139,0.15)]' : 'bg-black/40 text-slate-400 border-white/5 hover:border-white/10'}`}>{t('maintain')}</button>
                      <button onClick={() => setCalcGoal("gain")} className={`h-14 rounded-[1.25rem] text-xs font-black uppercase tracking-wider border transition-all duration-300 ${calcGoal === "gain" ? 'bg-indigo-500/20 text-indigo-400 border-indigo-500/50 shadow-[0_0_15px_rgba(99,102,241,0.15)]' : 'bg-black/40 text-slate-400 border-white/5 hover:border-white/10'}`}>{t('gain_muscle')}</button>
                   </div>
                </div>
             </div>
